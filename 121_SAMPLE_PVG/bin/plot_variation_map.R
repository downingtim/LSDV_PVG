...
library(ggplot2)
library(ggExtra)
map <- read.csv("../../../CURRENT/variation_map.txt", sep="\t")
colnames(map) <- c("sample", "allele1", "pos1", "allele2", "pos2")
pdf("CURRENT/variation_map-basic.pdf")
plot(map$pos1, map$pos2, cex=0.05) +grid()
dev.off()
...
pdf("../../../CURRENT/variation_map.pdf", width=13, height=6)
