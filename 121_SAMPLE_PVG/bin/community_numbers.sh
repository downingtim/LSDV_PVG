...
output_file="../../../COMMUNITIES/genomes.communities.mash.paths.txt"
...
for i in {0..3}; do
    ...
    if [[ -f "../../../COMMUNITIES/genomes.mapping.paf.edges.weights.txt.community.${i}.txt" ]]; then
        # Extract unique chromosome names from the community file
        chromosomes=$(cut -f 3 -d '#' "../../../COMMUNITIES/genomes.mapping.paf.edges.weights.txt.community.${i}.txt" | sort | uniq | tr '\n' ' ')
